...
/* 
* Every handling of GET/POST requests of different types and using the authorization middleware first.
...
/*
* Check if a source with the same name already exists for the user. If it does, return a response with status code of 409 and a message indicating
* that a source with that name already exists. If it does not, create a new Source object with the provided data and the id of the authenticated
* user and saves it to the database. If the save is successful, return a response with success set to true.
*/
...
/*
* Check if the source object with the provided id exists and if it is owned by the authenticated user. If it does not or is not owned by 
* the authenticated user, the route sends a response with a 409 status code and an error message. If it does, check if there is already 
* a source object with the same name owned by the authenticated user and if it does exist, send a response with status set to 409 and an error message.
...
* return a JSON object with success set to true. Find a single source in the database that matches the specified id and owner s and remove it. 
...
* provided name and userId, respectively. If no matching source is found, return a response with status of 409 and a message indicating 
* that the source was not found. If a matching source is found, create a new object called source and populate it with the type, url, login,
* passcode and vhost of the found source. Then return a response with success set to true and the source object.
*/
router.post('/source',
  async (req, res, next) => {
    try {
